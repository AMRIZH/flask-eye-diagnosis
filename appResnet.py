...
# Modify the fc layer for 5 classes
model.fc = torch.nn.Linear(model.fc.in_features, 5)
model.load_state_dict(torch.load(
    './static/models/coin_resnet50.pth', map_location=torch.device('cpu')))
model.eval()  # Set the model to evaluation mode

# Labels for the coin classes
labels = ['100', '1000', '200', '50', '500']
...
def results():
    ...
    try:
        ...
        top_class_index = torch.argmax(probabilities, dim=1).item()
        top_class_name = labels[top_class_index]
        top_class_probability = float(probabilities[0][top_class_index])

        # Get all class probabilities
        all_classes = {labels[i]: float(probabilities[0][i])
